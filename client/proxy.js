...
function buildSelfSigned() {
  ...
  return run('mkdir -p certs/{server,client,ca,tmp}').then(() => {
    ...
    fqdn = out.substring('SHA256 Fingerprint='.length)
        .split(':').join('').toLowerCase().trim().substring(0, 32) + '.' + DNS_ZONE;
    ...
    // Determine the fingerprint of the signing cert
    return run('openssl x509 -in certs/server/my-server.crt.pem -sha1 -noout -fingerprint');
  }).then(out => {
...
function buildPublicLocal(fqdn) {
  ...
  return getLocalIPAddr().then(localIpAddr => {
    var cmd = `./update.sh ${hash} ${localIpAddr} ${TUNNEL_IPADDR}`;
...
function proxyPublicLocal(hash) {
  ...
  https.createServer({
    key: fs.readFileSync(`certs/${hash}.${DNS_ZONE}/privkey.pem`),
    cert: fs.readFileSync(`certs/${hash}.${DNS_ZONE}/cert.pem`),
    ca: fs.readFileSync(`certs/${hash}.${DNS_ZONE}/chain.pem`)
  }, (req, res) => {
